...
func (p *parser) eatComment() bool {
	...
	if p.i < p.len-1 && p.s[p.i] == '#' && IsCommentFirstSpace(p.s[p.i+1]) {
		...
		p.tokens = append(p.tokens, ast.Token{Type: ast.COMMENT, Span: NodeSpan{start, p.i}, Raw: string(p.s[start:p.i])})
		return true
	} else {
		return false
	}
...
loop:
	for p.i < p.len {
		...
		case ' ', '\t', '\r':
		case '\n':
			p.tokens = append(p.tokens, ast.Token{Type: ast.NEWLINE, Span: NodeSpan{p.i, p.i + 1}})
			linefeedCount++
		default:
			break loop
		}
