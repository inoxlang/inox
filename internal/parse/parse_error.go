...
const (
	...
	INVALID_STRING_INTERPOLATION_SHOULD_NOT_BE_EMPTY         = "string interpolation should not empty"
	...
	UNTERMINATED_OPION_PATTERN_A_VALUE_IS_EXPECTED                  = "unterminated option pattern, a value is expected after a '='"
	...
	INVALID_SENDVALUE_EXPRESSION_MISSING_TO_KEYWORD_BEFORE_RECEIVER = "invalid send value expression: missing 'to' keyword before receiver value"
	...
	UNTERMINATED_RECEP_HANDLER_MISSING_HANDLER_OR_PATTERN = "unterminated reception handler expression: missing handler or patterb"
	...
	INVALID_WATCH_EXP_ONLY_SIMPLE_CALLES_ARE_SUPPORTED                 = "invalid watch expression: only simple calls are supported for now"
	...
	INVALID_LIST_PATT_GENERAL_ELEMENT_IF_ELEMENTS           = "invalid list pattern literal, the general element (after ']') should not be specified if elements are specified"
	...
	UNTERMINATED_OPENING_XML_TAG_MISSING_CLOSING       = "unterminated opening xml tag: missing closing '>'"
...
func fmtSlashDotDotDotCanOnlyBePresentAtEndOfPathPattern(value string) string {
	return fmt.Sprintf("'/...' can only be present at the end of a path pattern '%s'", value)
}
...
func fmtInvalidQueryKeysCannotContaintDollar(key string) string {
	return fmt.Sprintf("invalid query: keys cannot contain '$' or '{': key '%s'", key)
}
...
func fmtInvalidConstDeclMissingEqualSign(name string) string {
	return fmt.Sprintf("invalid global const declaration, missing '=' after name %s", name)
}

func fmtInvalidLocalVarDeclMissingEqualSign(name string) string {
	return fmt.Sprintf("invalid local variable declaration, missing '=' after name %s", name)
}

func fmtFuncNameShouldBeAnIndentNot(identLike Node) string {
