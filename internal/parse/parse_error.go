...
const (
	UnspecifiedParsingError = "UnspecifiedParsingError"
	MissingExpr             = "MissingExpr"

	UnterminatedOptionExpr = "UnterminatedOptionExpr"

	UnterminatedMemberExpr                 = "UnterminatedMemberExpr"
	UnterminatedDoubleColonExpr            = "UnterminatedDoubleColonExpr"
	UnterminatedExtendStmt                 = "UnterminatedExtendStmt"
	UnterminatedPatternDefinition          = "UnterminatedPatternDefinition"
	UnterminatedPatternNamespaceDefinition = "UnterminatedPatternNamespaceDefinition"
	UnterminatedStructDefinition           = "UnterminatedStructDefinition"

	UnterminatedSwitchStmt = "UnterminatedSwitchStmt"
	UnterminatedSwitchExpr = "UnterminatedSwitchExpr"
	UnterminatedMatchStmt  = "UnterminatedMatchStmt"
	UnterminatedMatchExpr  = "UnterminatedMatchExpr"

	UnterminatedWalkStmt = "UnterminatedWalkStmt"
	UnterminatedWalkExpr = "UnterminatedWalkExpr"

	UnterminatedForExpr = "UnterminatedForExpr"

	MissingBlock      = "MissingBlock"
	UnterminatedBlock = "UnterminatedBlock"

	UnterminatedArrow = "UnterminatedArrow"

	UnterminatedUnquotedRegion = "UnterminatedUnquotedRegion"

	UnterminatedObjectDestructuration = "UnterminatedObjectDestructuration"

	MissingFnBody                  = "MissingFnBody"
	MissingEqualsSignInDeclaration = "MissingEqualsSignInDeclaration"
	MissingObjectPropertyValue     = "MissingObjectPropertyValue"
	MissingObjectPatternProperty   = "MissingObjectPatternProperty"
	ExtractionExpressionExpected   = "ExtractionExpressionExpected"
...
const (
	...
	INVALID_SCHEME_LIT_SLASH_EXPECTED                          = "invalid scheme literal: a slash '/' is expected after ':/'"
	...
	INVALID_SCHEME_HOST_OR_URL_SLASH_EXPECTED                  = "invalid scheme, host or URL: a slash '/' is expected after ':/'"
	...
	CREDENTIALS_NOT_ALLOWED_IN_HOST_PATTERN_LITERALS           = "credentials (user:password) are not allowed in host pattern literals"
	...
	PARAM_LIST_OF_FUNC_SHOULD_BE_FOLLOWED_BY_BLOCK_OR_ARROW_OR_TYPE = "function: parameter list should be followed by a block, an arrow, or a return type"
	...
	//markup expression and markup pattern
	UNTERMINATED_MARKUP_EXPRESSION_MISSING_TOP_ELEM_NAME              = "unterminated markup expression: missing name of top element"
	UNTERMINATED_MARKUP_PATTERN_EXPRESSION_MISSING_TOP_ELEM_NAME      = "unterminated markup pattern expression: missing name of top element"
	UNTERMINATED_OPENING_MARKUP_TAG_MISSING_CLOSING                   = "unterminated opening markup tag: missing closing delimiter '>'"
	UNTERMINATED_SELF_CLOSING_MARKUP_TAG_MISSING_CLOSING              = "unterminated self-closing markup tag: missing closing '>' after '/'"
	UNTERMINATED_MARKUP_INTERP                                        = "unterminated markup interpolation"
	UNTERMINATED_CLOSING_MARKUP_TAG_MISSING_CLOSING_DELIM             = "unterminated closing markup tag: missing closing delimiter '>' after tag name"
	UNTERMINATED_HYPERSCRIPT_ATTRIBUTE_SHORTHAND                      = "unterminated hyperscript attribute shorthand"
	EMPTY_MARKUP_INTERP                                               = "markup interpolation should not be empty"
	INVALID_MARKUP_INTERP                                             = "invalid markup interpolation"
	MARKUP_INTERP_SHOULD_CONTAIN_A_SINGLE_EXPR                        = "an markup interpolation should contain a single expression"
	MARKUP_ATTRIBUTE_NAME_SHOULD_BE_IDENT                             = "markup attribute's name should be an identifier"
...
func fmtOnlyIdentsAndStringsValidObjRecordKeysNot(v ast.Node) string {
	var s string
	if lit, ok := v.(*ast.UnquotedStringLiteral); ok {
		s = "(" + lit.Value + ")"
	}
	return fmt.Sprintf("Only identifiers and strings are valid object/record literal keys, not a(n) %T %s", v, s)
}

func fmtOnlyIdentsAndStringsValidObjPatternKeysNot(v ast.Node) string {
	var s string
	if lit, ok := v.(*ast.UnquotedStringLiteral); ok {
		s = "(" + lit.Value + ")"
	}
...
func fmtFuncNameShouldBeAnIdentNot(identLike ast.Node) string {
...
func fmtInvalidForStmtKeyIndexVarShouldBeFollowedByVarNot(keyIndexIdent ast.Node) string {
...
func fmtInvalidForExprKeyIndexVarShouldBeFollowedByVarNot(keyIndexIdent ast.Node) string {
...
func fmtInvalidSpreadElemExprShouldBeExtrExprNot(expr ast.Node) string {
...
func fmtInvalidAssignmentInvalidLHS(expr ast.Node) string {
