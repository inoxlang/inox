...
var (
	SCHEMES = []string{"http", "https", "ws", "wss", "file", "mem", "s3"}
...
func (p *parser) parseURLLike(start int32, hostVariable *ast.Variable) ast.Node {
...
loop:
	...
	if hostVariable == nil && p.i == afterSchemeIndex {
		...
		if scheme == "" {
			parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_SCHEME_LIT_MISSING_SCHEME}
		}

		return &ast.SchemeLiteral{
			NodeBase: ast.NodeBase{
				Span: span,
				Err:  parsingErr,
			},
	...
	case LOOSE_HOST_REGEX.MatchString(u):
		...
		var parsingErr *sourcecode.ParsingError
		if err != nil {
			parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, err.Error()}
		}

		return &ast.HostLiteral{
			NodeBase: ast.NodeBase{
				Span: span,
				Err:  parsingErr,
			},
	...
	case LOOSE_URL_EXPR_REGEX.MatchString(u) && (hostVariable != nil || strings.Count(u, "{") >= 1): //url expressions
		...
		if hostVariable == nil && p.s[afterSchemeIndex] == '{' { //host interpolation
			p.tokens = append(p.tokens, ast.Token{
				...
				SubType: ast.HOST_INTERP_OPENING_BRACE,
			...
			p.tokens = append(p.tokens, ast.Token{
				...
				SubType: ast.HOST_INTERP_CLOSING_BRACE,
		...
		queryParams := make([]ast.Node, 0)
		if hasQuery { //parse query

			_, err := url.ParseQuery(string(p.s[pathExclEnd+1 : start+int32(len(u))]))
			if err != nil {
				parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_QUERY}
			}
			...
			for j < queryEnd {
				...
				if j >= queryEnd {
					parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, fmtInvalidQueryMissingEqualSignAfterKey(string(p.s[keyStart:j]))}
				}
				...
				if containsNotEscapedBracket(keyRunes) || containsNotEscapedDollar(keyRunes) {
					parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, fmtInvalidQueryKeysCannotContaintDollar(string(p.s[keyStart:j]))}
				}
				...
				queryParams = append(queryParams, &ast.URLQueryParameter{
					NodeBase: ast.NodeBase{
						NodeSpan{keyStart, j},
						nil,
						false,
		...
		var hostPart ast.Node
		hostPartString := string(p.s[span.Start:pathStart])
		hostPartBase := ast.NodeBase{
		...
		if hostInterpolationStart > 0 {
			...
			hostPart = &ast.HostExpression{
				...
				Scheme: &ast.SchemeLiteral{
					NodeBase: ast.NodeBase{NodeSpan{span.Start, afterSchemeIndex}, nil, false},
					Name:     string(p.s[span.Start : afterSchemeIndex-3]),
				},
				Host: e,
				Raw:  hostPartString,
			}
			ast.ShiftNodeSpans(e, hostInterpolationStart+1)

			if !ok && parsingErr == nil {
				parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_HOST_INTERPOLATION}
			}
		...
		return &ast.URLExpression{
			NodeBase:    ast.NodeBase{span, parsingErr, false},
			Raw:         u,
			HostPart:    hostPart,
			Path:        slices,
	...
	case LOOSE_URL_REGEX.MatchString(u): //urls & url patterns
		...
		if err != nil {
			return &ast.InvalidURL{
				NodeBase: ast.NodeBase{
					Span: span,
					Err:  &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_URL},
				},
				Value: u,
			}
		}
		...
		if err != nil {
			parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_URL + ": " + err.Error()}
		}

		if strings.Contains(parsed.Hostname(), "..") {
			parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_URL}
		}
...
func (p *parser) parseQueryParameterValueSlices(start int32, exclEnd int32) []ast.Node {
	p.panicIfContextDone()
	...
	for index < exclEnd {
		...
		case !inInterpolation && p.s[index] == '{':
			p.tokens = append(p.tokens, ast.Token{
				Type:    ast.OPENING_CURLY_BRACKET,
				SubType: ast.QUERY_PARAM_INTERP_OPENING_BRACE,
				Span:    NodeSpan{index, index + 1},
			})

			slice := string(p.s[sliceStart:index]) //previous cannot be an interpolation
			slices = append(slices, &ast.URLQueryParameterValueSlice{
				NodeBase: ast.NodeBase{
		...
		case inInterpolation && (p.s[index] == '}' || index == exclEnd-1):
			...
			if index == exclEnd-1 && p.s[index] != '}' {
				...
				p.tokens = append(p.tokens, ast.Token{
					Type:    ast.CLOSING_CURLY_BRACKET,
					SubType: ast.QUERY_PARAM_INTERP_CLOSING_BRACE,
					Span:    NodeSpan{index, index + 1},
				})
			}
			...
			if !ok {
				...
				err := &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_QUERY_PARAM_INTERP}
				...
				if missingClosingBrace {
					badSlice := &ast.URLQueryParameterValueSlice{
						NodeBase: ast.NodeBase{
...
func (p *parser) parseURLLikePattern(start int32, percentPrefixed bool) ast.Node {
	p.panicIfContextDone()
...
loop:
	...
	if leadingSlashCount != 2 {
		if !percentPrefixed && strings.HasSuffix(u, ":/") && strings.Count(u, "/") == 1 {
			return &ast.InvalidURL{
				NodeBase: ast.NodeBase{
					Span: NodeSpan{start, p.i},
					Err:  &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_SCHEME_LIT_SLASH_EXPECTED},
				},
				Value: u,
			}
		}
		return &ast.InvalidURLPattern{
			NodeBase: ast.NodeBase{
				Span: NodeSpan{start, p.i},
				Err:  &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_URL_OR_HOST_PATT_SCHEME_SHOULD_BE_FOLLOWED_BY_COLON_SLASH_SLASH},
			},
			Value: u,
		}
	}
	...
	case percentPrefixed && strings.HasSuffix(raw, "://"):
		return &ast.HostPatternLiteral{
			NodeBase: ast.NodeBase{
				Span: span,
				Err:  &sourcecode.ParsingError{UnspecifiedParsingError, UNTERMINATED_HOST_PATT_MISSING_HOSTNAME},
			},
			Value:      u,
			Raw:        raw,
			Unprefixed: !percentPrefixed,
	...
	case !LOOSE_URL_REGEX.MatchString(u):
		parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_URL_PATT}
	default:
		parsingErr = CheckURLPattern(u)
	}

	return &ast.URLPatternLiteral{
		NodeBase: ast.NodeBase{
			Span: span,
			Err:  parsingErr,
		},
...
func CheckHost(u string) error {
	...
	if err != nil ||
		...
		parsed.RawQuery != "" ||
		parsed.RawFragment != "" {
		return &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_HOST_LIT}
	}
...
func CheckHostPattern(u string) (parsingErr *sourcecode.ParsingError) {
	hasScheme := u[0] != ':'
	...
	if len32(parts) == 1 {
		if parts[0] != "**" {
			if parts[0] == "*" {
				parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_HOST_PATT_SUGGEST_DOUBLE_STAR}
			} else if _, err := url.ParseRequestURI(u); err != nil {
				parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_HOST_PATT}
			}
		}
	} else if strings.Count(u, "**") > 1 {
		parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_HOST_PATT_AT_MOST_ONE_DOUBLE_STAR}
	} else if strings.Count(u, "***") != 0 {
		parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_HOST_PATT_ONLY_SINGLE_OR_DOUBLE_STAR}
	} else {
		areAllStars := true
	...
	if parsingErr == nil {
		...
		if err != nil {
			...
			_, err = CheckGetEffectivePort(parsed.Scheme, parsed.Port())
			if err != nil {
				parsingErr = &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_HOST_PATT + ": " + err.Error()}
			}

			if parsed.User.String() != "" {
				return &sourcecode.ParsingError{UnspecifiedParsingError, CREDENTIALS_NOT_ALLOWED_IN_HOST_PATTERN_LITERALS}
			}
		}

		if parsingErr == nil && strings.Contains(parsed.Hostname(), "..") {
			return &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_HOST_PATT}
		}
...
func CheckURLPattern(u string) *sourcecode.ParsingError {
	...
	if strings.Contains(u, "...") && (!isPrefixPattern || strings.Count(u, "...") != 1) {
		...
		for _, r := range u[lastSlashI+1:] {
			if r == '.' {
				if c >= 3 {
					return &sourcecode.ParsingError{UnspecifiedParsingError, URL_PATTERNS_CANNOT_END_WITH_SLASH_MORE_THAN_4_DOTS}
				}
				c++
			}
		}

		return &sourcecode.ParsingError{UnspecifiedParsingError, URL_PATTERN_SUBSEQUENT_DOT_EXPLANATION}
	}
	...
	parsed, err := url.ParseRequestURI(replaced)
	if err != nil {
		return &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_URL_PATT}
	} else {
		_, err = CheckGetEffectivePort(parsed.Scheme, parsed.Port())
		if err != nil {
			return &sourcecode.ParsingError{UnspecifiedParsingError, INVALID_URL_PATT + ": " + err.Error()}
		}
	}
