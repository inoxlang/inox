...
func Print(node ast.Node, chunk *ast.Chunk, w io.Writer, config PrintConfig) (int, error) {
	tokens := ast.GetTokens(node, chunk, false)
	singleSpace := []byte{' '}
	...
	for i, token := range tokens {
		...
		if token.Type == ast.NEWLINE && isLeadingNewline && !config.KeepLeadingSpace {
...
func SPrint(node ast.Node, chunk *ast.Chunk, config PrintConfig) string {
	buff := bytes.Buffer{}
