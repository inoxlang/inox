...
func EvaluatePermissionListingObjectNode(n *ast.ObjectLiteral, config PreinitArgs) (*Object, error) {

	var state *TreeWalkState

	{
		var checkErr []error
		staticcheck.CheckPermissionListingObject(n, func(n ast.Node, msg string) {
...
func evaluateEnvSection(n *ast.ObjectPatternLiteral, state *TreeWalkState, m *Module) (*ObjectPattern, error) {
	v, err := TreeWalkEval(n, state)
	if err != nil {
		if err != nil {
...
func estimatePermissionsFromListingNode(permDescriptions *ast.ObjectLiteral) ([]Permission, error) {
	perms := make([]Permission, 0)
...
func estimatePartialPermissionNodeValue(n ast.Node) (Serializable, bool) {
	switch node := n.(type) {
