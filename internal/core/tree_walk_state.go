...
type TreeWalkState struct {
	...
	postHandle   func(node ast.Node, val Value, err error) (Value, error)
...
func (state *TreeWalkState) pushImportedChunk(chunk *parse.ParsedChunkSource, importNode *ast.InclusionImportStatement) {
...
func (state *TreeWalkState) popImportedChunk() {
	...
	if len(state.fullChunkStack) != 0 {
		state.currentFullChunkStackItem().CurrentNodeSpan = sourcecode.NodeSpan{}
	}
}

func (state *TreeWalkState) pushChunkOfCall(chunk *parse.ParsedChunkSource, callingNode ast.Node) {
...
func (state *TreeWalkState) updateStackTrace(currentStmt ast.Node) {
