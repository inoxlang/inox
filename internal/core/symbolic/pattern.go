...
type Pattern interface {
	...
	Call(ctx *Context, values []Value, optionalCallLocationNode ast.Node) (Pattern, error)
...
func (NotCallablePatternMixin) Call(ctx *Context, values []Value, optionalCallLocationNode ast.Node) (Pattern, error) {
...
func NewPathPatternFromNode(n ast.Node, chunk *ast.Chunk) *PathPattern {
...
func NewUrlPatternFromNode(n ast.Node, chunk *ast.Chunk) *URLPattern {
...
func NewHostPatternFromNode(n ast.Node, chunk *ast.Chunk) *HostPattern {
	printConfig := parse.PrintConfig{}
	return &HostPattern{
		node:            n,
		stringifiedNode: parse.SPrint(n, chunk, printConfig),
...
type NamedSegmentPathPattern struct {
	node *ast.NamedSegmentPathPatternLiteral //if nil, any node is matched
...
func NewNamedSegmentPathPattern(node *ast.NamedSegmentPathPatternLiteral) *NamedSegmentPathPattern {
...
func (p *NamedSegmentPathPattern) MatchGroups(v Value) (yes, possible bool, groups map[string]Serializable) {
	...
	if p.node != nil {
		for _, s := range p.node.Slices {
			segment, ok := s.(*ast.NamedPathSegment)
			if ok {
...
func (p *ObjectPattern) PrettyPrint(w pprint.PrettyPrintWriter, config *pprint.PrettyPrintConfig) {
	...
	if p.entries != nil {
		...
		for i, k := range keys {
			...
			//value
			v := p.entries[k]
			v.PrettyPrint(w.IncrIndent().EnterPattern(), config)
...
func prettyPrintListPattern(
	...
	if generalElementPattern != nil {
		...
		w.WritePercentString(s)

		generalElementPattern.PrettyPrint(w.EnterPattern(), config)
...
func (p *ListPattern) PrettyPrint(w pprint.PrettyPrintWriter, config *pprint.PrettyPrintConfig) {
	...
	w.WritePercentString("[]")
	p.generalElement.PrettyPrint(w.EnterPattern(), config)
}
...
func (p *UnionPattern) PrettyPrint(w pprint.PrettyPrintWriter, config *pprint.PrettyPrintConfig) {
	...
	for i, case_ := range p.cases {
		...
		case_.PrettyPrint(w.IncrIndent(), config)
	}
	w.WriteString(")")
}
...
func evalPatternNode(n ast.Node, state *State) (Pattern, error) {
	switch node := n.(type) {
	case *ast.ObjectPatternLiteral,
		*ast.RecordPatternLiteral,
		*ast.ListPatternLiteral,
		*ast.TuplePatternLiteral,
		...
		*ast.OptionalPatternExpression,

		*ast.AbsolutePathPatternLiteral, *ast.RelativePathPatternLiteral,
		*ast.NamedSegmentPathPatternLiteral,
		*ast.URLPatternLiteral, *ast.HostPatternLiteral,
		*ast.PathPatternExpression,
		*ast.ReadonlyPatternExpression:
...
type TypePattern struct {
	...
	call                func(ctx *Context, values []Value, optionalNode ast.Node) (Pattern, error)
...
func NewTypePattern(
	value Value, call func(ctx *Context, values []Value, optionalNode ast.Node) (Pattern, error),
...
func (p *TypePattern) PrettyPrint(w pprint.PrettyPrintWriter, config *pprint.PrettyPrintConfig) {
	...
	w.WriteName("type-pattern(")
	p.val.PrettyPrint(w.IncrIndent(), config)
	w.WriteString(")")
}
...
func (p *TypePattern) Call(ctx *Context, values []Value, optionalCallLocationNode ast.Node) (Pattern, error) {
	if p.call == nil {
		return nil, ErrPatternNotCallable
	}
...
func (p *DifferencePattern) PrettyPrint(w pprint.PrettyPrintWriter, config *pprint.PrettyPrintConfig) {
	...
	w.WriteString("(")
	p.Base.PrettyPrint(w.IncrIndent(), config)
	w.WriteString(" \\ ")
	p.Removed.PrettyPrint(w.IncrIndent(), config)
	w.WriteString(")")
}
...
func (fn *FunctionPattern) PrettyPrint(w pprint.PrettyPrintWriter, config *pprint.PrettyPrintConfig) {
	...
	w.WriteName("function-pattern(")
	fn.function.PrettyPrint(w.ZeroIndent(), config)
	w.WriteString(")")
}
...
func (p *IntRangePattern) PrettyPrint(w pprint.PrettyPrintWriter, config *pprint.PrettyPrintConfig) {
	...
	w.WriteByte('(')
	p.intRange.PrettyPrint(w.ZeroIndent(), config)
	w.WriteByte(')')
}
...
func (p *FloatRangePattern) PrettyPrint(w pprint.PrettyPrintWriter, config *pprint.PrettyPrintConfig) {
	...
	w.WriteByte('(')
	p.floatRange.PrettyPrint(w.ZeroIndent(), config)
	w.WriteByte(')')
}
