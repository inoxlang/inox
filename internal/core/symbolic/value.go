...
func (s *RecTestCallState) StartCallWithName(name string) {
	...
	if s.evalState != nil {
		s.evalState.testCallLocationBuffer.WriteString(name)
		s.currentTesterNameLen = len(name)
	}
}
...
func (s *RecTestCallState) FinishCall() {
	...
	if s.evalState != nil {
		buff := s.evalState.testCallLocationBuffer

		keptByteCount := buff.Len() - len(s.currentTesterName)
		s.evalState.testCallLocationBuffer.Truncate(keptByteCount)
	}
}

// ForProperty returns a new RecTestCallState with <dot> <$name> added to the call message buffer.
...
// ForProperty returns a new RecTestCallState with <dot> <$name> added to the call message buffer.
func (s RecTestCallState) ForIndex(name string) RecTestCallState {
	new := s

	if s.evalState != nil {
		buff := s.evalState.testCallMessageBuffer

		new.partNameLen = 2 /* '[' and ']' */ + len(name)
