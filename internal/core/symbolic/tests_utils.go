...
func MakeTestStateAndChunk(code string, globals ...map[string]Value) (*ast.Chunk, *State) {
...
func MakeTestStateAndChunks(code string, includedFiles map[string]string, globals ...map[string]Value) (*ast.Chunk, *State) {
	node, state, err := _makeStateAndChunk(code, globals...)
	if err != nil {
		panic(err)
	}

	state.Module.inclusionStatementMap = make(map[*ast.InclusionImportStatement]*IncludedChunk, len(includedFiles))

	for file, content := range includedFiles {
		importStmt := ast.FindNode(state.Module.mainChunk.Node, (*ast.InclusionImportStatement)(nil),
			func(stmt *ast.InclusionImportStatement, _ bool, _ []ast.Node) bool {
				pathLit, ok := stmt.Source.(*ast.RelativePathLiteral)
...
func MakeTestStateAndImportedModules(code string, files map[string]string, globals ...map[string]Value) (*ast.Chunk, *State) {
	node, state, err := _makeStateAndChunk(code, globals...)
	if err != nil {
		panic(err)
	}
