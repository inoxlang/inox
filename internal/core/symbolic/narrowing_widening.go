...
func narrow(positive bool, n ast.Node, state *State, targetState *State) {

	if unaryExpr, ok := n.(*ast.UnaryExpression); ok && unaryExpr.Operator == ast.BoolNegate {
		positive = !positive
		n = unaryExpr.Operand
	}
	...
	if boolConvExpr, ok := n.(*ast.BooleanConversionExpression); ok {
		if positive {
			narrowChain(boolConvExpr.Expr, removePossibleValue, Nil, targetState, 0)
		}
	}

	if binExpr, ok := n.(*ast.BinaryExpression); ok && state.symbolicData != nil {
		switch {
		case binExpr.Operator == ast.Match:
		...
		// (==) or negated (!=)
		case (positive && binExpr.Operator == ast.Equal) || (!positive && binExpr.Operator == ast.NotEqual):
		...
		// (!=) or negated (==)
		case (positive && binExpr.Operator == ast.NotEqual) || (!positive && binExpr.Operator == ast.Equal):
			//we narrow one of the operands
...
// element (a variable).
func narrowChain(chain ast.Node, action chainNarrowing, value Value, state *State, ignored int) {
...
switch_:
	...
	case *ast.DoubleColonExpression:
		//almost same logic as ast.MemberExpression
