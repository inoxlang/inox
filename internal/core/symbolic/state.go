...
type State struct {
	...
	//TODO: testedProgram *TestedProgram //can be nil
}
...
func (state *State) pushChunk(chunk *parse.ParsedChunkSource, stmt *ast.InclusionImportStatement) {
...
func (state *State) setGlobal(name string, value Value, constness GlobalConstness, optDefinitionNode ...ast.Node) (ok bool) {
...
func (state *State) overrideGlobal(name string, value Value, optDefinitionNode ...ast.Node) (ok bool) {
	scope := state.scopeStack[0]
...
func (state *State) setLocal(name string, value Value, static Pattern, optDefinitionNode ...ast.Node) {
	state.assertHasLocals()
...
func (state *State) updateLocal(name string, value Value, node ast.Node) bool {
...
func (state *State) narrowLocal(name string, value Value, node ast.Node) bool {
	ok, _ := state.updateLocal2(name, node, func(expected Value) (Value, bool, error) {
		return value, false, nil
	}, true)
	return ok
...
func (state *State) updateGlobal(name string, value Value, node ast.Node) bool {
...
func (state *State) narrowGlobal(name string, value Value, node ast.Node) bool {
	ok, _ := state.updateGlobal2(name, node, func(expected Value) (Value, bool, error) {
		return value, false, nil
	}, true)
	return ok
...
func (state *State) getInfoOfNode(partialNode ast.Node) (static Pattern, ofConstantVar bool, ok bool) {
...
func (state *State) getStaticOfNode(partialNode ast.Node) (Pattern, bool) {
...
func (state *State) addLocatedSymbolicGoFunctionError(msg string, location ast.Node) {
	state.tempSymbolicGoFunctionErrors = append(state.tempSymbolicGoFunctionErrors, symbolicGoFunctionError{
		message:  msg,
		location: location,
	})
...
func (state *State) consumeSymbolicGoFunctionErrors(fn func(msg string, optionalLocaction ast.Node)) {
