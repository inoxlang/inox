...
type SequenceStringPattern struct {
	SerializableMixin
	NotCallablePatternMixin
	node            *ast.ComplexStringPatternPiece //if nil any sequence string pattern is matched
	stringifiedNode string                         //empty if node is nil
}

func NewSequenceStringPattern(node *ast.ComplexStringPatternPiece, chunk *ast.Chunk) *SequenceStringPattern {
