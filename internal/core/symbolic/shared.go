...
func checkNotClonedObjectPropMutation(path ast.Node, state *State, propAssignment bool) {
	_checkNotClonedObjectPropDeepMutation(path, state, true, propAssignment, "")
}

func _checkNotClonedObjectPropDeepMutation(path ast.Node, state *State, first bool, propAssignment bool, elementNameHelp string) {
	...
	getNodeValue := func(n ast.Node) Value {
