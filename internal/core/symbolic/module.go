...
type Module struct {
	...
	directlyImportedModules map[*ast.ImportStatement]*Module
}

func NewModule(
	chunk *parse.ParsedChunkSource,
	inclusionStatementMap map[*ast.InclusionImportStatement]*IncludedChunk,
	importedModuleMap map[*ast.ImportStatement]*Module,
) *Module {
	return &Module{
		mainChunk:               chunk,
...
func (mod *Module) GetLineColumn(node ast.Node) (int32, int32) {
...
func (m *Module) SetProp(state *State, node ast.Node, name string, value Value) (IProps, error) {
...
func getModuleParameters(manifestObject *Object, manifestObjectLiteral *ast.ObjectLiteral) []ModuleParameter {
	...
	parametersObjectNode, ok := parametersNode.(*ast.ObjectLiteral)
