...
type InoxFunction struct {
	node           ast.Node //optional but required for call evaluation
	nodeChunk      *ast.Chunk
	parameters     []Value
	...
	visitCheckNode           func(visit visitArgs, globalsAtCreation map[string]Value) (ast.TraversalAction, bool, error)
...
type visitArgs struct {
	node, parent, scopeNode ast.Node
	ancestorChain           []ast.Node
	after                   bool
}
	...
	if fn.visitCheckNode != nil {
		...
		ast.Walk(
			...
			func(node, parent, scopeNode ast.Node, ancestorChain []ast.Node, after bool) (ast.TraversalAction, error) {
				if _, isBody := node.(*ast.Block); isBody && node == body {
...
func (goFunc *GoFunction) Call(input goFunctionCallInput) (finalResult Value, multipleResults bool, enoughArgs bool, finalErr error) {
	...
	for paramIndex := 0; paramIndex < nonVariadicParamCount; paramIndex++ {
		...
		case *ast.MarkupExpression:
			argumentNodes = []ast.Node{c.Element}
		}
...
type Function struct {
	...
	patternNode                  *ast.FunctionPatternExpression //can be nil
	patternNodeChunk             *ast.Chunk                     //can be nil
