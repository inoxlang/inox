...
type inoxCallInfo struct {
	calleeFnExpr       *ast.FunctionExpression
	callNode           ast.Node //nil if is initial check call
	isInitialCheckCall bool
}

type symbolicFunctionCall struct {
	callNode   ast.Node //not necessarily a *ast.CallExpression
	calleeNode ast.Node
	state      *State

	argNodes  []ast.Node //ignored if argValues != nil
	argValues []Value
...
func callSymbolicFunc(functionCall symbolicFunctionCall) (Value, error) {
	...
	var (
		...
		err             error
		self            Value
		selfPartialNode ast.Node //not necessarily a node from the evaluated code
	)

	//we first get the callee
	switch c := calleeNode.(type) {
	case *ast.IdentifierLiteral, *ast.IdentifierMemberExpression,
		*ast.Variable, *ast.MemberExpression, *ast.DoubleColonExpression:
		callee, err = _symbolicEval(calleeNode, state, evalOptions{
			doubleColonExprAncestorChain: []ast.Node{callNode},
		})
		if err != nil {
			return nil, err
		}
		...
		case *ast.IdentifierMemberExpression:
			...
			default:
				...
				selfPartialNode = &ast.IdentifierMemberExpression{
					Left:          _c.Left,
	...
	if ok {
		...
		varInfo, ok := state.getGlobal(inoxFunctionToBeDeclared.decl.Name.(*ast.IdentifierLiteral).Name)
		if !ok {
			return ANY, fmt.Errorf("error while evaluating the function declaration: %w", err)
		}
	...
	if functionCall.argValues != nil {
		...
		for argIndex, argNode := range argNodes {
			...
			if spreadArg, ok := argNode.(*ast.SpreadArgument); ok {
				hasSpreadArg = true
				...
				if ident, ok := argNode.(*ast.IdentifierLiteral); ok && cmdLineSyntax { //Identifier literal interpreted as an identifier value.
					...
					if state.hasLocal(ident.Name) || state.hasGlobal(ident.Name) {
						...
						if calleeIdent, ok := calleeNode.(*ast.IdentifierLiteral); !ok {
							addWarning = true
	...
	var fnExpr *ast.FunctionExpression
	...
	case *GoFunction:
		...
		state.consumeSymbolicGoFunctionErrors(func(msg string, optionalLocation ast.Node) {
			var location ast.Node = callNode
		...
		for i, arg := range currentArgs {
			...
			if i < nonSpreadArgCount && argNodes != nil { //argNodes may be nil if the call does not correspond to a *ast.CallExpression.
				argNode = argNodes[i]
			}
			...
			if !paramType.Test(arg, RecTestCallState{evalState: state.resetTestCallMsgBuffers()}) {
				if argNode != nil {
					...
					if _, ok := argNode.(*ast.RuntimeTypeCheckExpression); ok {
						args[i] = paramType
	...
	var (
		...
		parameterNodes          []*ast.FunctionParameter
		variadicParamNode       *ast.FunctionParameter
	)
	...
	for i, p := range parameterNodes[:nonVariadicParamCount] {
		name := p.Var.(*ast.IdentifierLiteral).Name
	...
	if isVariadic {
		...
		name := variadicParamNode.Var.(*ast.IdentifierLiteral).Name
...
func setAllowedNonPresentProperties(argNodes []ast.Node, nonSpreadArgCount int, params []Value, state *State) {
	//ignore spread arg
	argNodes = argNodes[:min(len(argNodes), nonSpreadArgCount)]
	//ignore additional arguments
	argNodes = argNodes[:min(len(argNodes), len(params))]

	removePropertiesAlreadyPresent := func(allowedNonPresentProperties []string, propNodes []*ast.ObjectProperty) []string {
	...
	for i, arg := range argNodes {
		...
		case *Object:
			...
			objLit, ok := arg.(*ast.ObjectLiteral)
			if !ok {
				continue
			}
		...
		case *Record:
			...
			recordLit, ok := arg.(*ast.RecordLiteral)
			if !ok {
				continue
			}
...
func checkTransformMustCallReturnValue(ret Value, callNode ast.Node, calleeNode ast.Node, state *State) Value {
...
func checkCallExprWithUnhandledError(node ast.Node, ret Value, state *State) {
	...
	if ident, ok := callExpr.Callee.(*ast.IdentifierLiteral); ok && ident.Name == globalnames.ERROR_FN {
		return
	}
