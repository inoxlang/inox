...
func init() {
	...
	staticcheck.EvalSimpleValueLiteral = func(node parse.SimpleValueLiteral) (any, error) {
	...
	staticcheck.GetCheckImportedModuleSourceName = func(sourceNode parse.Node, currentModule *inoxmod.Module, ctx inoxmod.Context) (string, error) {
		switch sourceNode.(type) {
		case *parse.URLLiteral, *parse.AbsolutePathLiteral, *parse.RelativePathLiteral:
			value, err := EvalSimpleValueLiteral(sourceNode.(parse.SimpleValueLiteral), nil)
			if err != nil {
...
func RegisterStaticallyCheckHostDefinitionFn(scheme Scheme, fn StaticallyCheckHostDefinitionFn) {
	staticcheck.RegisterStaticallyCheckHostDefinitionFn(scheme, func(optionalProject staticcheck.Project, node parse.Node) (errorMsg string) {
		if optionalProject == nil {
			return fn(nil, node)
		}
		return fn(optionalProject.(Project), node)
	})
}
...
func RegisterStaticallyCheckDbResolutionDataFn(scheme Scheme, fn StaticallyCheckDbResolutionDataFn) {
	staticcheck.RegisterDbResolutionDataCheckFn(scheme, func(node parse.Node, optionalProject staticcheck.Project) (errorMsg string) {
		if optionalProject == nil {
			return fn(node, nil)
		}
		return fn(node, optionalProject.(Project))
	})
}
