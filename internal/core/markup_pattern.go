...
func NewMarkupPatternFromExpression(node *ast.MarkupPatternExpression, bridge StateBridge) (*MarkupPattern, error) {
...
func newMarkupPatternElementFromNode(node *ast.MarkupPatternElement, bridge StateBridge) (*MarkupPatternElement, error) {
	...
	if node.RawElementContent != "" {
		...
		for _, child := range node.Children {
			...
			case *ast.MarkupPatternWildcard:
...
type NewMarkupPatternElementParameters struct {
	TagName    string
	Quantifier ast.MarkupPatternElementQuantifier
	Attributes map[string]StringPattern
...
func evalNodeInMarkupPattern(e ast.Node, bridge StateBridge) (Value, error) {
	switch e := e.(type) {
...
func (patt *MarkupPatternElement) testChildren(ctx *Context, node ImmutableMarkupNode, pool *sync.Pool) bool {
	...
	for patternIndex, childPattern := range patt.children {
		...
		match_elements:
			for child != nil {
				...
				if childPattern.Test(ctx, child, pool, isPrevChildPatternWildcard, isNextChildPatternWildcard) {
					...
					case ast.OneMarkupElement, ast.OptionalMarkupElement:
						matchCount = 1
						break match_elements
					}
					matchCount++
			...
			switch childPattern.quantifier {
			case ast.OneMarkupElement, ast.OneOrMoreMarkupElements:
				if matchCount == 0 {
					return false
				}
			default:
		...
		isPrevChildPatternWildcard = utils.Implements[*ast.MarkupWildcard](childPattern)
	}
