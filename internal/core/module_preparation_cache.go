...
func (c *PreparationCacheEntry) MainChunkTopLevelNodeIs(chunk *ast.Chunk) bool {
	if c.module == nil {
		return false
	}
...
func (c *PreparationCacheEntry) CheckValidity() bool {
	c.lock.Lock()
	defer c.lock.Unlock()

	if c.haveChunkChanged(c.module.MainChunk) {
		return false
	}

	for _, importedMod := range c.module.DirectlyImportedModules {
		if c.haveChunkChanged(importedMod.MainChunk) {
			return false
		}
	}

	for _, includedChunk := range c.module.InclusionStatementMap {
		if c.haveChunkChanged(includedChunk.ParsedChunkSource) {
			return false
		}
	}
...
func (c *PreparationCacheEntry) haveChunkChanged(chunk *parse.ParsedChunkSource) bool {
	...
	if srcFile.IsResourceURL {
		...
		if stat, err := os.Stat(name); err != nil || stat.ModTime().After(c.time) {
			return true
		}
	}
