...
func NewUnionPattern(cases []Pattern, node ast.Node) *UnionPattern {
	cases = flattenUnionPatternCases(cases, false, 0)
	return &UnionPattern{node: node, cases: cases}
}

func NewDisjointUnionPattern(cases []Pattern, node ast.Node) *UnionPattern {
...
func NewIntersectionPattern(cases []Pattern, node ast.Node) *IntersectionPattern {
...
func (patt *ObjectPattern) Test(ctx *Context, v Value) bool {
	...
	for _, constraint := range patt.complexPropertyPatterns {
		...
		if err != nil {
			if ctx != nil {
				ctx.LoggerPrint("error when checking a complex property pattern: " + err.Error())
			}
			//TODO: log error some where
			return false
		}
		if b, ok := res.(Bool); !ok {
			ctx.LoggerPrint("error when checking a multiproperty pattern")
		} else if !b {
			return false
		}
	}
...
type FunctionPattern struct {
	node      *ast.FunctionPatternExpression //if nil, matches any function
	nodeChunk *ast.Chunk
