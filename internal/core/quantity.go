...
func mustEvalQuantityRange(n *ast.QuantityRangeLiteral) QuantityRange {
	lowerBound := utils.Must(evalQuantity(n.LowerBound.Values, n.LowerBound.Units))

	var upperBound Serializable
	if n.UpperBound != nil {
		upperBound = utils.Must(evalQuantity(n.UpperBound.(*ast.QuantityLiteral).Values, n.UpperBound.(*ast.QuantityLiteral).Units))
	} else {
