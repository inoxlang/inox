...
func checkSingleKindPermissions(permKind permbase.PermissionKind, desc parse.Node, onError func(n parse.Node, msg string)) {
...
func CheckDatabasesObject(
	...
	onValidDatabase func(name string, scheme Scheme, resource inoxmod.ResourceName), //optional
	project Project,
) {
	...
	if onValidDatabase == nil {
		onValidDatabase = func(name string, scheme Scheme, resource inoxmod.ResourceName) {}
	}
	...
	for _, p := range obj.Properties {
		...
		var scheme Scheme
		var resource inoxmod.ResourceName
		...
		for _, prop := range dbDesc.Properties {
			...
			case inoxconsts.MANIFEST_DATABASE__RESOURCE_PROP_NAME:
				...
				case *parse.HostLiteral:
					...
					if u != nil {
						scheme = CreateScheme(u.Scheme)
						resource = utils.Must(EvalSimpleValueLiteral(res)).(Host)
					}
				case *parse.URLLiteral:
					u, _ := url.Parse(res.Value)
					if u != nil {
						scheme = CreateScheme(u.Scheme)
						resource = utils.Must(EvalSimpleValueLiteral(res)).(URL)
					}
				default:
	...
	for _, p := range obj.Properties {
		...
		case inoxconsts.MANIFEST_INVOCATION__ON_ADDED_ELEM_PROP_NAME:
			if urlLit, ok := p.Value.(*parse.URLLiteral); ok {
				...
				if err == nil {
					if !IsStaticallyCheckDBFunctionRegistered(CreateScheme(scheme)) {
