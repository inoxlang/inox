...
type sharedDebuggerFields struct {
	...
	breakpoints            map[sourcecode.NodeSpan]BreakpointInfo
...
func NewDebugger(args DebuggerArgs) *Debugger {
	...
	initialBreakpoints := map[sourcecode.NodeSpan]BreakpointInfo{}
	...
	for _, breakpoint := range args.InitialBreakpoints {
		...
		if breakpoint.NodeSpan != (sourcecode.NodeSpan{}) {
...
func (d *Debugger) beforeInstruction(n ast.Node, trace []StackFrameInfo, exceptionError error) {
	if d.closed.Load() {
		return
	}
...
func ParseFileChunk(absoluteSourcePath string, opts ...parse.ParserOptions) (*parse.ParsedChunkSource, error) {
	content, err := ReadFileInFS(absoluteSourcePath, -1)
	if err != nil {
