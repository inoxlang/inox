...
func NewStringFromSlices(slices []Value, node *ast.StringTemplateLiteral, ctx *Context) (String, error) {
...
func NewCheckedString(slices []Value, node *ast.StringTemplateLiteral, ctx *Context) (*CheckedString, error) {
	patternIdent, isPatternAnIdent := node.Pattern.(*ast.PatternIdentifierLiteral)
	...
	if isPatternAnIdent {
		...
		namespaceMembExpr := node.Pattern.(*ast.PatternNamespaceMemberExpression)
