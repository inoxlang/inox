...
var (
	...
	VAL_PATTERN = &TypePattern{
		...
		SymbolicCallImpl: func(ctx *symbolic.Context, values []symbolic.Value, optionalNode ast.Node) (symbolic.Pattern, error) {
		...
		SymbolicCallImpl: func(ctx *symbolic.Context, values []symbolic.Value, optionalNode ast.Node) (symbolic.Pattern, error) {
			...
			if !ok {
				if optionalNode != nil {
					...
					state := ctx.EvalState()
					msg, regions := symbolic.FmtInvalidArg(state.FormatHelper(), 0, values[0], symbolic.ANY_PATTERN, nil)
					return nil, symbolic.MakeSymbolicEvalError(optionalNode, state, msg, regions...)
				}
				//Return an error without any location or regions.
				msg, _ := symbolic.FmtInvalidArg(nil, 0, values[0], symbolic.ANY_PATTERN, nil)
			...
			for _, val := range values {
				...
				default:
					if optionalNode != nil {
						//Return a symbolic error.
						state := ctx.EvalState()
						msg, regions := symbolic.FmtInvalidArg(state.FormatHelper(), 0, v, symbolic.ANY_OBJECT_PATTERN, nil)
						return nil, symbolic.MakeSymbolicEvalError(optionalNode, state, msg, regions...)
					}
					//Return an error without any location or regions.
					msg, _ := symbolic.FmtInvalidArg(nil, 0, values[0], symbolic.ANY_PATTERN, nil)
			...
			for _, val := range values {
				...
				default:
					if optionalNode != nil {
						//Return a symbolic error.
						state := ctx.EvalState()
						msg, regions := symbolic.FmtInvalidArg(state.FormatHelper(), 0, v, symbolic.ANY_TUPLE, nil)
						return nil, symbolic.MakeSymbolicEvalError(optionalNode, state, msg, regions...)
					}
					//Return an error without any location or regions.
					msg, _ := symbolic.FmtInvalidArg(nil, 0, values[0], symbolic.ANY_PATTERN, nil)
	...
	EVENT_PATTERN = &TypePattern{
		...
		SymbolicCallImpl: func(ctx *symbolic.Context, args []symbolic.Value, _ ast.Node) (symbolic.Pattern, error) {
			...
			for _, val := range values {
				...
				default:
					if optionalNode != nil {
						//Return a symbolic error.
						state := ctx.EvalState()
						msg, regions := symbolic.FmtInvalidArg(state.FormatHelper(), 0, v, symbolic.ANY_STR_PATTERN, nil)
						return nil, symbolic.MakeSymbolicEvalError(optionalNode, state, msg, regions...)
					}
					//Return an error without any location or regions.
					msg, _ := symbolic.FmtInvalidArg(nil, 0, values[0], symbolic.ANY_PATTERN, nil)
