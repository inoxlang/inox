...
func testEval(t *testing.T, bytecodeEval bool, Eval evalFn) {
	...
	t.Run("sequence string pattern", func(t *testing.T) {
		...
		t.Run("single element: integer range with no upper bound", func(t *testing.T) {
			...
			expectedPattern := core.NewIntRangeStringPattern(
				1,
				math.MaxInt64,
				ast.FindNode(state.Module.MainChunk.Node, (*ast.IntegerRangeLiteral)(nil), nil),
			)
	...
	allExtendStmtTestsPassed := t.Run("extend statement", func(t *testing.T) {
		t.Run("computed property", func(t *testing.T) {
			...
			extendStmt, ancestors := ast.FindNodeAndChain(state.Module.MainChunk.Node, (*ast.ExtendStatement)(nil), nil)
		...
		assert.EqualValues(t, core.AstNode{
			Node: &ast.IntLiteral{
				NodeBase: ast.NodeBase{
					Span:            sourcecode.NodeSpan{Start: 2, End: 3},
					IsParenthesized: true,
					// Tokens: []ast.Token{
					// 	{Type: ast.OPENING_PARENTHESIS, Span: sourcecode.NodeSpan{Start: 1, End: 2}},
					// 	{Type: ast.CLOSING_PARENTHESIS, Span: sourcecode.NodeSpan{Start: 3, End: 4}},
					// },
				},
				Raw:   "1",
				Value: 1,
	...
	t.Run("mapping expression", func(t *testing.T) {
		...
		// 				`{"int__value":1}`: ast.FindNode(mod, &ast.StaticMappingEntry{}, nil),
		// 			},
		// 			dynamicEntries: map[string]*ast.DynamicMappingEntry{
		// 				`{"int__value":2}`: ast.FindNode(mod, &ast.DynamicMappingEntry{}, nil),
		// 			},
		// 			patterns: []struct {
		// 				string
		// 				core.Pattern
		...
		// 			},
		// 			dynamicEntries: map[string]*ast.DynamicMappingEntry{
		// 				intTypePatternRepr: ast.FindNode(mod, &ast.DynamicMappingEntry{}, nil),
		// 			},
		// 			patterns: []struct {
		// 				string
		// 				core.Pattern
	...
	// 	makeSourceFile := func(code string) ast.SourceFile {
	...
	// 		chunk := ast.MustParseChunk(src.CodeString)
	// 		testcaseNode := chunk.Statements[0].(*ast.TestSuiteExpression).Module.Statements[0].(*ast.TestCaseExpression)
	...
	// 		chunk := ast.MustParseChunk(src.CodeString)
	// 		testcaseNode := chunk.Statements[0].(*ast.TestSuiteExpression).
	// 			Module.Statements[2].(*ast.TestSuiteExpression).
	// 			Module.Statements[0].(*ast.TestCaseExpression)
	...
	// 		chunk := ast.MustParseChunk(src.CodeString)
	// 		testsuiteNode := chunk.Statements[0].(*ast.TestSuiteExpression).Module.Statements[1].(*ast.TestSuiteExpression)
		...
		t.Run("element", func(t *testing.T) {
			...
			expectedPattern := core.NewMarkupPattern(core.NewMarkupPatternElement(
				core.NewMarkupPatternElementParameters{
					TagName:    "div",
					Quantifier: ast.OneMarkupElement,
	...
	t.Run("error position stack", func(t *testing.T) {
		...
		t.Run("in included file", func(t *testing.T) {
			...
			binExpr := ast.FindNode(includedChunk.Node, (*ast.BinaryExpression)(nil), nil)
		...
		t.Run("in an included file (deep)", func(t *testing.T) {
			...
			importStmt1 := ast.FindNode(mod.MainChunk.Node, (*ast.InclusionImportStatement)(nil), nil)
			importStmt2 := ast.FindNode(includedChunk1.Node, (*ast.InclusionImportStatement)(nil), nil)
			binExpr := ast.FindNode(includedChunk2.Node, (*ast.BinaryExpression)(nil), nil)
		...
		t.Run("in a function defined by an included file but called by the module", func(t *testing.T) {
			...
			includedChunk := mod.IncludedChunkForest[0]
			callExpr := ast.FindNode(mod.MainChunk.Node, (*ast.CallExpression)(nil), nil)
			fnDecl := ast.FindNode(includedChunk.Node, (*ast.FunctionDeclaration)(nil), nil)
			binExpr := ast.FindNode(includedChunk.Node, (*ast.BinaryExpression)(nil), nil)
		...
		t.Run("in a function defined by an included file (deep) but called by the module", func(t *testing.T) {
			...
			callExpr := ast.FindNode(mod.MainChunk.Node, (*ast.CallExpression)(nil), nil)
			fnDecl := ast.FindNode(includedChunk2.Node, (*ast.FunctionDeclaration)(nil), nil)
			binExpr := ast.FindNode(includedChunk2.Node, (*ast.BinaryExpression)(nil), nil)
