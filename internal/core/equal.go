...
//Implementations of Value.Equal for Core value types.
...
func (list KeyList) Equal(ctx *Context, other Value, alreadyCompared map[uintptr]uintptr, depth int) bool {
	...
	for _, v := range list {
		if !slices.Contains(otherList, v) {
			return false
		}
	}
...
func (o *Opaque) Equal(ctx *Context, other Value, alreadyCompared map[uintptr]uintptr, depth int) bool {
	other, ok := other.(*Opaque)
	return ok && other == o
}
