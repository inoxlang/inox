...
type frame struct {
	...
	currentNodeSpan sourcecode.NodeSpan
}

func (f frame) GetCurrentNodeSpan() (sourcecode.NodeSpan, bool) {
	return f.currentNodeSpan, f.currentNodeSpan != sourcecode.NodeSpan{}
}
...
func NewVM(config VMConfig) (*VM, error) {
	...
	if inoxFn == nil {
		...
		if self != nil {
			return nil, errors.New("cannot set self")
		}
	} else {
		runFn = true
	...
	for atomic.LoadInt64(&v.aborting) == 0 {
		...
		case OpReturn:
			...
			if !topLevelFnEval {
				...
				if !v.runFn {
					v.curFrame.currentNodeSpan = sourcecode.NodeSpan{}
					v.chunkStack[len(v.chunkStack)-1].CurrentNodeSpan = sourcecode.NodeSpan{}
				} else {
		...
		case OpSetGlobal:
			...
			err := v.global.Globals.SetCheck(string(globalName), val, func(defined bool) error {
				perm := GlobalVarPermission{Kind_: permbase.Create, Name: string(globalName)}
				if defined {
					perm.Kind_ = permbase.Update
				}
		...
		case OpCreateObject:
			...
			astNode := v.constants[astNodeIndex].(AstNode).Node.(*ast.ObjectLiteral)
		...
		case OpCreateMapping:
			...
			node := v.constants[nodeIndex].(AstNode).Node.(*ast.MappingExpression)
		...
		case OpCreateOrderedPair:
			...
			v.sp--
		case OpCreateModuleArgs:
			v.ip += 3
			paramsIndex := int(v.curInsts[v.ip-1]) | int(v.curInsts[v.ip-2])<<8
			numElements := int(v.curInsts[v.ip])

			params := v.constants[paramsIndex].(*ModuleParamsPattern)

			entries := map[string]Value{}
			for i := v.sp - 2*numElements; i < v.sp; i += 2 {
				name := v.stack[i].(String)
				entries[string(name)] = v.stack[i+1]
			}
		...
		case OpCreateString:
			...
			nodeIndex := int(v.curInsts[v.ip]) | int(v.curInsts[v.ip-1])<<8
			node := v.constants[nodeIndex].(AstNode).Node.(*ast.StringTemplateLiteral)
...
func (v *VM) doSafeIntBinOp() {
	...
	operator := ast.BinaryOperator(v.curInsts[v.ip])
...
func (v *VM) handleOtherOpcodes(op byte) (_continue bool) {
	...
	case OpCreateRepeatedPatternElement:
		v.ip += 2
		quantifier := ast.SequencePatternQuantifier(v.curInsts[v.ip-1])
		exactOccurenceCount := int(v.curInsts[v.ip])

		patternElement := v.stack[v.sp-1].(StringPattern)

		v.stack[v.sp-1] = newRepeatedPatternElement(quantifier, exactOccurenceCount, patternElement)
	case OpCreateSequenceStringPattern:
		...
		val, err := NewSequenceStringPattern(node.Node.(*ast.ComplexStringPatternPiece), node.Chunk_.Node, subpatterns, nameList)
		if err != nil {
			v.err = err
			return
		}
	...
	case OpPushIncludedChunk:
		...
		inclusionStmt := v.constants[inclusionStmtIndex].(AstNode).Node.(*ast.InclusionImportStatement)
	...
	case OpCreateXMLelem:
		...
		var attributes []NonInterpretedMarkupAttribute
		if attributeCount > 0 {
			attributes = make([]NonInterpretedMarkupAttribute, attributeCount)

			attributesStart := v.sp - childCount - 2*attributeCount
			for i := 0; i < 2*attributeCount; i += 2 {
				attributes[i/2] = NewMarkupAttribute(
					string(v.stack[attributesStart+i].(String)),
					v.stack[attributesStart+i+1],
				)
			}
		}

		var elem *NonInterpretedMarkupElement

		if rawContent != Nil {
			elem = NewNonInterpretedRawTextMarkupElement(tagName, attributes, string(rawContent.(String)))
		} else {
			...
			elem = NewNonInterpretedMarkupElement(tagName, attributes, children)
		}
	...
	case OpCallFromXMLFactory:
		xmlElem := v.stack[v.sp-2].(*NonInterpretedMarkupElement)

		ns := v.stack[v.sp-1].(*Namespace)
		factory := ns.Prop(v.global.Ctx, symbolic.FROM_MARKUP_FACTORY_NAME).(*GoFunction)
	...
	// 	embeddedModChunk := embeddedChunkNode.Node.(*ast.Chunk)
	// 	meta := v.stack[v.sp-1]

	// 	suite, err := NewTestSuite(TestSuiteCreationInput{
	// 		Meta:             meta,
	// 		Node:             node.Node.(*ast.TestSuiteExpression),
	...
	// 	embeddedModChunk := modNode.Node.(*ast.Chunk)
	// 	parentChunk := node.chunk
	// 	meta := v.stack[v.sp-1]
	...
	// 		Meta: meta,
	// 		Node: node.Node.(*ast.TestCaseExpression),
	...
	case OpImport:
		...
		varPerm := GlobalVarPermission{permbase.Create, string(globalName)}
	...
	case OpSpawnLThread:
		...
		if meta != nil && meta != Nil {
			...
			globalsSection, ok := metaMap[symbolic.LTHREAD_META_GLOBALS_SECTION].(*ModuleArgs)
			var explicitlyPassedGlobals map[string]Value
			if ok {
				explicitlyPassedGlobals = globalsSection.ValueMap()
			}

			group, globalsDesc, permListing, v.err = readLThreadMeta(metaMap, explicitlyPassedGlobals, v.global.Ctx)
			if v.err != nil {
				return
			}
		}
	...
	case OpCreateAddTypeExtension:
		...
		extendStmt := v.constants[extendStmtIndex].(AstNode).Node.(*ast.ExtendStatement)
		...
		methodIndex := 0
		for _, prop := range extendStmt.Extension.(*ast.ObjectLiteral).Properties {
	...
	case OpRuntimeTypecheck:
		v.ip += 2
		astNodeIndex := int(v.curInsts[v.ip]) | int(v.curInsts[v.ip-1])<<8
		astNode := v.constants[astNodeIndex].(AstNode).Node.(*ast.RuntimeTypeCheckExpression)
	...
	case OpAssert:
		v.ip += 2
		nodeIndex := int(v.curInsts[v.ip]) | int(v.curInsts[v.ip-1])<<8
		stmt := v.constants[nodeIndex].(AstNode).Node.(*ast.AssertionStatement)

		ok := v.stack[v.sp-1].(Bool)
		v.sp--

		modKind := v.global.Module.Kind
