...
type compiler struct {
	...
	earlyFunctionDeclarations         []*ast.FunctionDeclaration
...
func (c *compiler) Compile(node ast.Node) error {
	...
	if c.earlyFunctionDeclarationsPosition >= 0 && node.Base().Span.Start >= c.earlyFunctionDeclarationsPosition {
		...
		for _, decl := range decls {
			if err := c.Compile(decl); err != nil {
				return fmt.Errorf("failed to compile early declaration of the function `%s`: %w", decl.Name.(*ast.IdentifierLiteral).Name, err)
			}
		}
	}
	...
	case *ast.GlobalConstantDeclaration:
	...
	case *ast.BinaryExpression:
		if node.Operator == ast.And || node.Operator == ast.Or {
		...
		case ast.Add, ast.Sub, ast.Mul, ast.Div:
			...
			switch {
			case symbolic.ImplOrMultivaluesImplementing[*symbolic.Int](left):
				c.emit(node, OpNumBin, int(node.Operator))
			case symbolic.ImplOrMultivaluesImplementing[*symbolic.Float](left):
				c.emit(node, OpNumBin, int(node.Operator))
			default:
		...
		case ast.AddDot, ast.SubDot, ast.MulDot, ast.DivDot, ast.LessThanDot, ast.LessOrEqualDot, ast.GreaterThanDot, ast.GreaterOrEqualDot:
			return errors.New("dot operators not supported yet")
		case ast.Range, ast.ExclEndRange:
			exclEnd := 0
			if node.Operator == ast.ExclEndRange {
				exclEnd = 1
			}
	...
	case *ast.URLExpression:
		...
		for _, pathSlice := range node.Path {
			_, isStaticPathSlice := pathSlice.(*ast.PathSlice)
		...
		for _, p := range node.QueryParams {
			...
			for i, n := range param.Value {
				...
				if _, ok := n.(*ast.URLQueryParameterValueSlice); !ok {
	...
	case *ast.ForExpression:
		...
		c.emit(node, OpIntBin, int(ast.Add))
		c.emit(node, OpSetLocal, listLengthSymbol.Index)
		...
		for i, case_ := range node.Cases {
			for j, valueNode := range case_.Values {
				...
				if case_.GroupMatchingVariable != nil {
					variable := case_.GroupMatchingVariable.(*ast.IdentifierLiteral)
	...
	case *ast.Block:
		...
		if len(node.Statements) > 1 {
			...
			if node.Statements[0].Kind() == ast.Expr {
				c.emit(node, OpPop)
			}
		}
	...
	case *ast.LocalVariableDeclarations:
		for _, decl := range node.Declarations {
			symbol := c.currentLocalSymbols().Define(decl.Left.(*ast.IdentifierLiteral).Name)
	...
	case *ast.GlobalVariableDeclarations:
		for _, decl := range node.Declarations {
			varname := decl.Left.(*ast.IdentifierLiteral).Name
	...
	case *ast.MultiAssignment:
		...
		for i, variable := range node.Variables {
			name := variable.(*ast.IdentifierLiteral).Name
	...
	case *ast.ListLiteral:
		...
		for i, elem := range node.Elements {
			if spreadElem, ok := elem.(*ast.ElementSpreadElement); ok {
	...
	case *ast.DictionaryLiteral:
		for _, entry := range node.Entries {
			...
			if lit, ok := entry.Key.(ast.SimpleValueLiteral); ok && !utils.Implements[*ast.IdentifierLiteral](lit) {
	...
	case *ast.DoubleColonExpression:
		...
		if ok {
			...
			for _, prop := range symbolicExtension.Statement.Extension.(*ast.ObjectLiteral).Properties {
				if prop.Name() != node.Element.Name {
					continue
				}

				_, ok := prop.Value.(*ast.FunctionExpression)
				if ok {
	...
	case *ast.FunctionDeclaration:
		funcName := node.Name.(*ast.IdentifierLiteral).Name
		_, exists := c.globalSymbols.Resolve(funcName)

		if exists { //Declared before the statement.
			return nil
		}

		s := c.globalSymbols.Define(funcName) //Define the symbol early in case the function is recursive.
		s.IsConstant = true
		...
		c.emit(node, OpSetGlobal, c.addConstant(String(funcName)))
	case *ast.FunctionExpression:
		...
		for _, p := range node.Parameters {
			currentLocals.Define(p.Var.(*ast.IdentifierLiteral).Name)
		}
		...
		if len(node.CaptureList) != 0 {
			...
			for _, e := range node.CaptureList {
				s, _ := locals.Resolve(e.(*ast.IdentifierLiteral).Name)
				c.emit(node, OpGetLocal, s.Index)
			}
	...
	case *ast.SpawnExpression:
		if node.Meta != nil {
			objLit := node.Meta.(*ast.ObjectLiteral)
			//we handle this case separately because objects cannot contain non-serializable values.

			metaParamTypes := map[string]Pattern{}

			for _, property := range objLit.Properties {
				propertyName := property.Name() //okay since implicit-key properties are not allowed
				metaParamTypes[propertyName] = ANYVAL_PATTERN

				c.emit(property, OpPushConstant, c.addConstant(String(propertyName)))

				if propertyName != symbolic.LTHREAD_META_GLOBALS_SECTION || !utils.Implements[*ast.ObjectLiteral](property.Value) {
					...
					globalsLit := property.Value.(*ast.ObjectLiteral)

					globalTypes := map[string]Pattern{}

					for _, prop := range globalsLit.Properties {
						globalName := prop.Name() //okay since implicit-key properties are not allowed
						globalTypes[globalName] = ANYVAL_PATTERN

						c.emit(prop, OpPushConstant, c.addConstant(String(globalName)))
						if err := c.Compile(prop.Value); err != nil {
							return err
						}
					}

					anonGlobalsStructType := NewModuleParamsPattern(globalTypes, ModuleParameters{})
					globalCount := len(globalsLit.Properties)
					c.emit(globalsLit, OpCreateModuleArgs, c.addConstant(anonGlobalsStructType), globalCount)
				}
			}

			anonStructType := NewModuleParamsPattern(metaParamTypes, ModuleParameters{})
			propCount := len(objLit.Properties)

			c.emit(node.Meta, OpCreateModuleArgs, c.addConstant(anonStructType), propCount)
		} else {
			c.emit(node, OpPushNil)
		}
		...
		case *ast.KeyListExpression:
			for _, key := range g.Keys {
				embeddedModCompiler.globalSymbols.Define(key.(*ast.IdentifierLiteral).Name)
			}
		case *ast.ObjectLiteral:
	...
	case *ast.ConcatenationExpression:
		...
		switch {
		case symbolic.ImplOrMultivaluesImplementing[*symbolic.ByteSlice](firstElemSymbValue):
			opCode = OpConcatBytesLikes
		case symbolic.ImplOrMultivaluesImplementing[symbolic.StringLike](firstElemSymbValue):
			opCode = OpConcatStrLikes
		case symbolic.ImplOrMultivaluesImplementing[*symbolic.Tuple](firstElemSymbValue):
	...
	case *ast.RuntimeTypeCheckExpression:
		...
		constantVal := AstNode{Node: node, Chunk_: c.currentChunk()}
	...
	case *ast.ExtendStatement:
		...
		objLit := node.Extension.(*ast.ObjectLiteral)
		methodCount := 0
...
func (c *compiler) compileAssignOperation(node *ast.Assignment, rhs ast.Node) error {
	...
	case ast.MinusAssign:
		...
		c.emit(node, OpIntBin, int(ast.Sub))
	case ast.MulAssign:
		if err := c.Compile(rhs); err != nil {
			return err
		}
		c.emit(node, OpIntBin, int(ast.Mul))
	case ast.DivAssign:
		if err := c.Compile(rhs); err != nil {
			return err
		}
		c.emit(node, OpIntBin, int(ast.Div))
	}

	return nil
}

func (c *compiler) compileAssign(node *ast.Assignment, lhs, rhs ast.Node) error {

	switch l := lhs.(type) {
	case *ast.Variable, *ast.IdentifierLiteral:
		var varname = ast.GetVariableName(lhs)
...
func (c *compiler) CompileVar(node *ast.IdentifierLiteral) error {
	isGlobal := true
...
func (c *compiler) CompileStringPatternNode(node ast.Node) error {
	switch v := node.(type) {
	case *ast.DoubleQuotedStringLiteral:
	...
	case *ast.ComplexStringPatternPiece:
		...
		for i, element := range v.Elements {
			...
			if element.Quantifier != ast.ExactlyOneOccurrence {
				c.emit(element, OpCreateRepeatedPatternElement, int(element.Quantifier), element.ExactOcurrenceCount)
			}
...
func (c *compiler) NewError(node ast.Node, msg string) error {
