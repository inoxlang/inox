...
type TreeWalkCall struct {
	...
	self               Value
	state              *TreeWalkState
	arguments          any //[]Value or []ast.Node
	must               bool
	cmdLineSyntax      bool
...
func TreeWalkCallFunc(call TreeWalkCall) (Value, error) {
	...
	if l, ok := arguments.([]Value); ok {
		...
		for argIndex, argn := range arguments.([]ast.Node) {

			if spreadArg, ok := argn.(*ast.SpreadArgument); ok {
				hasSpreadArg = true
				...
				if ident, ok := argn.(*ast.IdentifierLiteral); ok && cmdLineSyntax {
	...
	case *InoxFunction:
		...
		case *ast.FunctionDeclaration:
			fn = node.Function
			functionName = node.Name.(*ast.IdentifierLiteral).Name
		default:
	...
	if fn.IsVariadic {
		...
		name := fn.Parameters[len(fn.Parameters)-1].Var.(*ast.IdentifierLiteral).Name
