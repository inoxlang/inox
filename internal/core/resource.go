...
func ResourceNameFrom(s string) ResourceName {
	...
	switch n.(type) {
	case *ast.HostLiteral, *ast.AbsolutePathLiteral, *ast.RelativePathLiteral, *ast.URLLiteral:
		return utils.Must(EvalSimpleValueLiteral(n.(ast.SimpleValueLiteral), nil)).(ResourceName)
	}
...
func ParsePathLiteral(s string) (Path, bool) {
	...
	switch e.(type) {
	case *ast.AbsolutePathLiteral, *ast.RelativePathLiteral:
		path, err := EvalSimpleValueLiteral(e.(ast.SimpleValueLiteral), nil)
		if err != nil {
			return "", false
		}
...
func (pth Path) ToAbs() (Path, error) {
	if pth.IsAbsolute() {
		return pth, nil
	}
...
// /a , /b -> error
// /a , /b/ -> error
func (pth Path) Join(relativePath Path) Path {
...
// /a , ./b -> error
// /a , ./b/ -> error
func (pth Path) JoinAbsolute(absPath Path) Path {
	if !absPath.IsAbsolute() {
		panic(errors.New("path argument is not absolute"))
	}
	dirpath := Path(filepath.Join(string(pth), string(absPath)))
