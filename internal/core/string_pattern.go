...
type SequenceStringPattern struct {
	...
	node       *ast.ComplexStringPatternPiece //optional
	nodeChunk  *ast.Chunk                     //should be set if node is set
...
func NewSequenceStringPattern(
	node *ast.ComplexStringPatternPiece,
	nodeChunk *ast.Chunk,
...
func NewUnionStringPattern(node ast.Node, cases []StringPattern) (*UnionStringPattern, error) {
...
func NewRuneRangeStringPattern(lower, upper rune, node ast.Node) *RuneRangeStringPattern {
...
func NewIntRangeStringPattern(lower, upperIncluded int64, node ast.Node) *IntRangeStringPattern {
...
func NewFloatRangeStringPattern(lower, upperIncluded float64, node ast.Node) *FloatRangeStringPattern {
...
type RepeatedPatternElement struct {
	NotCallablePatternMixin
	regexp        *regexp.Regexp
	quantifier    ast.SequencePatternQuantifier
	exactCount    int
	element       StringPattern
	fullyResolved bool
}

func newRepeatedPatternElement(quantifier ast.SequencePatternQuantifier, exactOcurrenceCount int, repeated StringPattern) *RepeatedPatternElement {
...
func constructRegexForRepeatedPatternElement(
	ocurrenceModifier ast.SequencePatternQuantifier,
	exactCount int,
	...
	case ast.AtLeastOneOccurrence:
		buf.WriteRune('+')
	case ast.ZeroOrMoreOccurrences:
		buf.WriteRune('*')
	case ast.OptionalOccurrence:
		buf.WriteRune('?')
	case ast.ExactOccurrenceCount:
		buf.WriteRune('{')
...
func (patt *RepeatedPatternElement) validate(s string, i *int) bool {
	...
	if !patt.element.validate(s, &j) {
		ok = patt.quantifier == ast.ZeroOrMoreOccurrences || patt.quantifier == ast.OptionalOccurrence
	} else {
		count := 1
		...
		case ast.ExactlyOneOccurrence:
			ok = count == 1
		case ast.AtLeastOneOccurrence, ast.ZeroOrMoreOccurrences, ast.OptionalOccurrence:
			ok = true
		case ast.ExactOccurrenceCount:
...
type NamedSegmentPathPattern struct {
	NotCallablePatternMixin
	node *ast.NamedSegmentPathPatternLiteral
}
