...
const (
	...
	PREINIT_KEYWORD_STRING         = "preinit"
...
type Token struct {
	...
	Span    sourcecode.NodeSpan `json:"span"`
	Raw     string              `json:"raw"`
}
...
func GetTokens(node Node, chunk *Chunk, addMeta bool) []Token {
	...
	nodeBase := node.Base()
	startTokenIndex, _ := slices.BinarySearchFunc(chunkTokens, nodeBase.Span, func(t Token, ns sourcecode.NodeSpan) int {
	...
	Walk(node, func(node, parentNode, _ Node, ancestorChain []Node, _ bool) (TraversalAction, error) {
		...
		case *MemberExpression:
			...
			tokens = append(tokens, Token{
				Type: DOT,
				Span: sourcecode.NodeSpan{Start: i, End: i + 1},
			})

			if n.Optional {
				tokens = append(tokens, Token{
					Type: QUESTION_MARK,
					Span: sourcecode.NodeSpan{Start: i + 1, End: i + 2},
				})
			}
		...
		case *URLExpression:
			...
			if _, ok := n.HostPart.(*IdentifierLiteral); ok {
				tokens = append(tokens, Token{
					Type: AT_SIGN,
					Span: sourcecode.NodeSpan{Start: n.Span.Start, End: n.Span.Start + 1},
				})
			}
			...
			for i, r := range n.Raw {
				...
				case '?':
					tokens = append(tokens, Token{
						...
						Span: sourcecode.NodeSpan{n.Span.Start + int32(i), n.Span.Start + int32(i) + 1},
					})
				case '&':
					tokens = append(tokens, Token{
		...
		case *URLQueryParameter:
			...
			tokens = append(tokens, Token{
				...
				Span: sourcecode.NodeSpan{n.Span.Start, n.Span.Start + int32(utf8.RuneCountInString(n.Name)) + 1},
				Raw:  n.Name + "=",
			})
		...
		case *OptionalPatternExpression:
			tokens = append(tokens, Token{
				Type: QUESTION_MARK,
				Span: sourcecode.NodeSpan{n.Span.End - 1, n.Span.End},
			})
			return ContinueTraversal, nil
		...
		case *OptionExpression:
			...
			tokens = append(tokens, Token{
				Type: OPTION_NAME,
				Span: sourcecode.NodeSpan{n.Span.Start, nameEnd},
				Raw:  namePart,
			})
			tokens = append(tokens, Token{
				Type:    EQUAL,
				SubType: FLAG_EQUAL,
				Span:    sourcecode.NodeSpan{nameEnd, nameEnd + 1},
			})
		...
		case *MarkupElement:
			if n.RawElementContent != "" {
				...
				tokens = append(tokens, Token{
					...
					Span: sourcecode.NodeSpan{Start: start, End: end},
		...
		case *MarkupPatternOpeningTag:
			if n.Quantifier != 0 {
				...
				tokens = append(tokens, Token{
					...
					Raw:  raw,
					Span: sourcecode.NodeSpan{Start: nameSpan.End, End: nameSpan.End + 1},
				})
			}
		case *ObjectDestructurationProperty:
			if n.Nillable {
				tokens = append(tokens, Token{
					Type: QUESTION_MARK,
					Span: sourcecode.NodeSpan{Start: n.PropertyName.Span.End, End: n.PropertyName.Span.End + 1},
				})
			}
		}
	...
	for i := 0; i < len(tokens); i++ {
		...
		if token.Type <= LAST_TOKEN_TYPE_WITHOUT_VALUE {
			token.Raw = TOKEN_STRINGS[token.Type]
		}
...
func GetTokenAtPosition(pos int, node Node, chunk *Chunk) (Token, bool) {
	tokens := GetTokens(node, chunk, false)
	index, ok := slices.BinarySearchFunc(tokens, Token{Type: 0, Span: sourcecode.NodeSpan{Start: int32(pos), End: int32(pos + 1)}}, func(a, b Token) int {
