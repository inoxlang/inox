...
func TestNewNode(t *testing.T) {
	...
	t.Run("empty description", func(t *testing.T) {
		...
		//remove ids
		result.html.Walk(func(n html_ns.HTMLNode) error {
			n.RemoveAttribute(ctx, "id")
			return nil
		})

		result.originalDesc = html_ns.NodeDescription{}

		assert.Equal(t, &Node{
			html:  html_ns.NewNode(ctx, "div", core.NewObject()),
			model: core.Nil,
		}, result)
	})
...
func TestNewAutoNode(t *testing.T) {
	...
	t.Run("boolean", func(t *testing.T) {
		...
		assert.Equal(t, &Node{
			kind:  AutoNode,
			html:  html_ns.CreateTextNode(core.Str("true")),
			model: core.True,
		}, result)
	})
	...
	t.Run("value with user provided rendering config", func(t *testing.T) {
		...
		assert.Equal(t, &Node{
			kind:  AutoNode,
			html:  html_ns.CreateTimeElem(core.Str(dateString)),
	...
	t.Run("empty list", func(t *testing.T) {
		...
		assert.Equal(t, "<ul></ul>", string(html_ns.RenderToString(ctx, result)))
	})
	...
	t.Run("dynamic if", func(t *testing.T) {
		...
		t.Run("simple value | nil", func(t *testing.T) {
			...
			assert.Equal(t, "true", string(html_ns.RenderToString(ctx, result)))

			obj.SetProp(ctx, "cond", core.False)
			time.Sleep(WAIT_TIME_AFTER_MUTATION)
			assert.Equal(t, "<span></span>", string(html_ns.RenderToString(ctx, result)))
		...
		t.Run("nil | static DOM node", func(t *testing.T) {
			...
			obj := core.NewObjectFromMap(core.ValMap{"cond": core.True}, ctx)
			cond, _ := core.NewDynamicMemberValue(ctx, obj, "cond")
			childNode := NewStaticNode(html_ns.CreateSpanElem(core.Str("a")))
			...
			obj.SetProp(ctx, "cond", core.False)
			time.Sleep(WAIT_TIME_AFTER_MUTATION)
			assert.Equal(t, "<span>a</span>", string(html_ns.RenderToString(ctx, result)))
		...
		t.Run("auto DOM node that changes | nil", func(t *testing.T) {
			...
			assert.Equal(t, "<div></div>", string(html_ns.RenderToString(ctx, result)))

			childNodeModel.SetProp(ctx, "a", core.True)
			time.Sleep(WAIT_TIME_AFTER_MUTATION)
			assert.Equal(t, "<div><div>true</div></div>", string(html_ns.RenderToString(ctx, result)))
		})
	})
